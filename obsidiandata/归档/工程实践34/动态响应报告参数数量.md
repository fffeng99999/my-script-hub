...
该页面无法根据参数的数量动态响应列表的行数，请根据实际拿到的 增强质量的量化评估指标，绘制表格。
    ...
    <n-tabs type="line" animated>  
      <n-tab-pane name="compare" tab="单步结果对比">  
        ...
        <n-space justify="space-around">  
          ...
          </div>        </n-space>      </n-tab-pane>      <n-tab-pane name="gif" tab="GIF 动画">  
        <n-p depth="3">自动生成的处理全过程 GIF 动画。</n-p>  
        <n-space justify="center">  
          <n-image :src="gifUrl" />  
        </n-space>      </n-tab-pane>      <n-tab-pane name="metrics" tab="质量指标">  
        ...
        </n-descriptions>      </n-tab-pane>    </n-tabs>    <n-divider />    <n-h6>下载选项</n-h6>  
    <n-space>      <n-button          v-for="image in filteredComparisonImages.filter(img => !img.label.includes('原始'))"  
          :key="image.label"  
